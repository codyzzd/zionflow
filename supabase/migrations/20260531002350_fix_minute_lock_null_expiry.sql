...
set
  ...
  data = data - 'lockedByUserId' - 'lockedAt' - 'lockExpiresAt'
...
begin
  ...
  with updated as (
    ...
    where sm.id = p_minute_id
      and (
        sm.locked_by is null
        or sm.locked_by = p_user_id
        or sm.lock_expires_at is null
        or sm.lock_expires_at <= now()
      )
    returning sm.*
  ),
  current_row as (
...
begin
  ...
  with updated as (
    ...
    where sm.id = p_minute_id
      ...
      and (sm.lock_expires_at is null or sm.lock_expires_at > now())
    returning sm.*
  ),
  current_row as (
...
begin
  ...
  if v_current.locked_by is distinct from p_user_id
    or v_current.lock_expires_at is null
    or v_current.lock_expires_at <= v_now
  then
